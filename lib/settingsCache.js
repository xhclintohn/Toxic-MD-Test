const { getSettings, getSudoUsers, getBannedUsers, registerSettingsListener, registerSudoListener, registerBannedListener } = require('../src/database');
...
async function getCachedBanned() {
  ...
  async function getCachedAllowed() {
      const { getAllowedUsers } = require('../src/database');
      const now = Date.now();
